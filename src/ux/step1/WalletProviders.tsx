...
import {
  PROVIDERS_WRAPPER_CLASSNAME,
  PROVIDERS_DEVELOPER_CLASSNAME,
...
export const WalletProviders = ({
  ...
  const developerProviders = Object.keys(providersByName).filter(
    (providerName: string) =>
  ...
  return (
    <>
      <Header2>
        {Object.keys(userProviders).length !== 0 ? (
      ...
      <ProvidersWrapper className={PROVIDERS_WRAPPER_CLASSNAME}>
        <ProviderRow>
          <UserProvider
            userProvider={
              providersByName[providers.METAMASK.name] || providers.METAMASK
            }
        ...
        <ProviderRow hideMobile={true}>
          <UserProvider
            userProvider={
              providersByName[providers.WALLETCONNECT.name] ||
